...
import {
  lookupReuploadRequestByPublicId,
  ...
  switch (result.state) {
    case "expired":
      return {
        ...
        body: "새 재업로드 링크가 필요합니다. 페르패키지 담당자에게 다시 문의해 주세요."
      };
    case "used":
      return {
        title: "이미 재업로드가 완료된 요청입니다.",
    ...
    default:
      return {
        title: "재업로드 요청을 확인할 수 없습니다.",
        body: "링크가 잘못되었거나 더 이상 사용할 수 없습니다. 담당자에게 다시 문의해 주세요."
      };
  }
}

export default async function PublicReuploadRequestPage({ params }: PublicReuploadPageProps) {
  const publicId = params.publicId ?? "";
  const lookup = await lookupReuploadRequestByPublicId(publicId);
    ...
    return (
      <main className="grid">
        ...
        <section className="panel panel-pad">
          <h2>안내</h2>
          <p className="lead">
            재업로드 요청은 요청별로 발급되며, 만료되었거나 이미 사용된 링크는 다시 사용할 수 없습니다.
          </p>
          {lookup.request ? (
  ...
  return (
    <main className="grid">
      <section className="hero">
        ...
        <p className="lead">
          요청받은 수정 파일 1개만 업로드해 주세요. 여러 파일을 전달해야 하는 경우 하나의 ZIP 파일로 압축해 업로드해 주세요.
        </p>
      </section>
        ...
        <div className="notice" style={{ marginTop: 16 }}>
          ...
          <p style={{ marginBottom: 0, marginTop: 8 }}>
            {lookup.request.customer_message?.trim() ||
              "안내받은 수정 사항을 반영한 파일을 다시 업로드해 주세요."}
          </p>
        </div>
      </section>
      ...
      <section className="panel panel-pad">
        ...
        <div style={{ marginTop: 18 }}>
          <ReuploadCustomerUploadForm publicId={publicId} />
        </div>
      </section>
    </main>
  );
